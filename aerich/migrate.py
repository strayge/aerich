...
from aerich.utils import get_models_describe, is_default_function
...
class Migrate:
    upgrade_operators: List[Operation] = []
    downgrade_operators: List[Operation] = []
    ...
    async def init(cls, config: dict, app: str, location: str):
        ...
        if last_version:
            cls.last_version_number = int(last_version.version.split("_", 1)[0])
    ...
    async def generate_version(cls, name=None):
        ...
        filenames = cls.get_all_version_files()
        versions = [int(filename.split("_", 1)[0]) for filename in filenames]
        last_version = max(versions) if versions else None
        if last_version is None:
            return f"0_{now}_init.py"
        version = f"{last_version + 1}_{now}_{name}.py"
        if len(version) > MAX_VERSION_LENGTH:
            raise ValueError(f"Version name exceeds maximum length ({MAX_VERSION_LENGTH})")
        return version

    @classmethod
    async def _generate_migration_file(
        cls,
        name: str,
        ...
        write_operations(version_file, upgrade, downgrade)
    ...
    async def migrate(cls, name) -> str:
        ...
        :return:
        """
        new_models_content = get_models_describe(cls.app)
        old_models_content = make_model(
            [
                operation
                for name in cls.get_all_version_files()
                for operation in read_operations(cls.migrate_location / name)[0]
            ]
        )

        upgrade, downgrade = compare_models(old_models_content, new_models_content)
        ...
        # cls.diff_models(old_models_content, new_models_content)
        # cls.diff_models(new_models_content, old_models_content, False)
        # cls._merge_operations()

        if not upgrade:
            return ""

        return await cls._generate_migration_file(name, upgrade, downgrade)
    ...
    def diff_models(cls, old_models: Dict[str, dict], new_models: Dict[str, dict], upgrade=True):
        ...
        for new_model_str, new_model_describe in new_models.items():
            ...
            old_unique_together = set(
                map(lambda x: tuple(x), old_model_describe["unique_together"])
            )
            new_unique_together = set(
                map(lambda x: tuple(x), new_model_describe["unique_together"])
            )
            old_indexes = set(
                map(
            ...
            old_pk_field = old_model_describe["pk_field"]
            new_pk_field = new_model_describe["pk_field"]
            # pk field
                    ...
                    if add:
                        cls._add_operator(
                            cls.create_m2m(
                                model,
                                change[0][1],
                                new_models[change[0][1]["model_name"]],
                            ),
                            upgrade,
                            fk_m2m_index=True,
                        )
            ...
            old_data_fields = list(
                filter(
                    ...
                    old_model_describe["data_fields"],
                )
            )
            new_data_fields = list(
                filter(
                    lambda x: x.get("db_field_types") is not None,
                    new_model_describe["data_fields"],
                )
            )

            old_data_fields_name = list(map(lambda x: x["name"], old_data_fields))
            new_data_fields_name = list(map(lambda x: x["name"], new_data_fields))
            ...
            for new_data_field_name in set(new_data_fields_name).difference(
                ...
                new_data_field = next(
                    filter(lambda x: x["name"] == new_data_field_name, new_data_fields)
                )
                is_rename = False
                for old_data_field in old_data_fields:
                    changes = list(diff(old_data_field, new_data_field))
                    old_data_field_name = old_data_field["name"]
                    if len(changes) == 2:
                        # rename field
                        if (
                            changes[0]
            ...
            old_fk_fields = old_model_describe["fk_fields"]
            new_fk_fields = new_model_describe["fk_fields"]
            ...
            for new_fk_field_name in set(new_fk_fields_name).difference(set(old_fk_fields_name)):
                ...
                if fk_field.get("db_constraint"):
                    cls._add_operator(
                        cls._add_fk(model, fk_field, new_models.get(fk_field["python_type"])),
                        upgrade,
                        fk_m2m_index=True,
                    )
            # drop fk
            for old_fk_field_name in set(old_fk_fields_name).difference(set(new_fk_fields_name)):
                ...
                if old_fk_field.get("db_constraint"):
                    cls._add_operator(
                        cls._drop_fk(
                            model, old_fk_field, old_models.get(old_fk_field["python_type"])
                        ),
                        upgrade,
                        fk_m2m_index=True,
                    )
